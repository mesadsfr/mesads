...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        """We forgot to set the fields ads_type in load_ads_from_ille_et_villaine.py. This script overrides the field.
        """
        ...
        for dossier in self._iterate_demarches(**opts):
            ...
            ads_type = None
            if 'ancienne ADS' in fields[8]['stringValue']:
                ads_type = 'old'
            elif 'nouvelle ADS' in fields[8]['stringValue']:
                ads_type = 'new'

            if ads.ads_type and ads.ads_type != ads_type:
                self._log(
                    self.style.WARNING,
                    f'Dossier {dossier["number"]} has ads_type={ads_type}, but in database the type of ADS {ads.id} '
                    f'is set to {ads.ads_type}. Leave the database value as-is.'
                )
                continue
