...
def fix_data_from_hidden_forms(apps, schema_editor):
    ...
    for ads in ADS.objects.exclude(attribution_type="").filter(
    ...
    for ads in ADS.objects.exclude(attribution_reason="").filter(
        Q(ads_creation_date__isnull=False, ads_creation_date__gte=date(2014, 10, 1))
        | Q(ads_creation_date__isnull=True)
    ):
    ...
    for ads in ADS.objects.exclude(attribution_type="paid").exclude(
    ...
    for ads in (
        ADS.objects.exclude(owner_license_number="")
        .exclude(
            ads_creation_date__isnull=False, ads_creation_date__gte=date(2014, 10, 1)
        )
        .exclude(used_by_owner__isnull=False, used_by_owner=True)
    ):
